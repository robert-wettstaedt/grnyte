...
For this repository, enforce i18n in every delegated task that includes user-facing text:

1. Require agents to use the project translate function (for example `t(...)` from `getI18n()`) for all user-visible strings.
2. Require agents to check existing translation keys first in:

- `src/lib/i18n/locales/en.json`
- `src/lib/i18n/locales/de.json`

3. Require key reuse whenever possible.
4. Allow new keys only if no suitable key exists.
5. Require any new key to be added to both locale files.

## Project UI Framework Rules (Mandatory)

For this repository, enforce Skeleton for Svelte as the default UI framework in all delegated tasks:

1. Require UI tasks to use Skeleton + Tailwind patterns and Skeleton component conventions (`@skeletonlabs/skeleton-svelte`).
2. Require agents to avoid conflicting UI kits (for example DaisyUI or Flowbite).
3. If another UI library is necessary, require compatible/headless integrations and preserve Skeleton as the visual system.
4. For substantial UI work, require consulting Skeleton docs:
