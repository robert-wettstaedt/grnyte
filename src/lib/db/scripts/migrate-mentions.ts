 ...
 * grammar areas/blocks/routes already use). Run this alongside the deploy that
 * retires `remark-mentions`, so no legacy `@username` is left unresolved.
 *
 ...
 *
 * Usage:
 *   npx tsx src/lib/db/scripts/migrate-mentions.ts            # apply
 *   npx tsx src/lib/db/scripts/migrate-mentions.ts --dry-run  # preview only
 *
 ...
 *
 * Uses relative imports and a standalone postgres/drizzle client (like
 * `migrate.ts`) because the `$lib` alias isn't available under `tsx`.
 */
