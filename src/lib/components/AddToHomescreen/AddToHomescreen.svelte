<script lang="ts">
  import Dialog from '$lib/components/Dialog'
  import { getI18n } from '$lib/i18n'

  let { modalOpen = $bindable(false) } = $props()
  const { t } = getI18n()

  const iOSShareBase64 =
    ''
</script>

<Dialog open={modalOpen} onOpenChange={(event) => (modalOpen = event.open)} title={t('homescreen.addToHomeScreen')}>
  {#snippet content()}
    <p>{t('homescreen.iosInstructions')}</p>

    <ol class="mt-4 list-inside list-decimal">
      <li>
        {t('homescreen.clickBelow')} <img alt="Share" src={`data:image/png;base64,${iOSShareBase64}`} class="inline" />
      </li>
      <li>
        <strong>{t('homescreen.chooseAddToHomeScreen')}</strong>
      </li>
    </ol>
  {/snippet}
</Dialog>
