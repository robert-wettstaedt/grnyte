<!--
  ...
  16px target with no padding, and the panel could not be animated. `::details-content` plus
  `interpolate-size: allow-keywords` is the native answer to the second half and is Chrome
  only (no Firefox, no Safari as of 2026-08), which rules it out for a mobile app. So this is
  the same button-and-slide disclosure the card's own change list uses, which also puts the
  reader on one idiom rather than two that behave differently.
-->
<script lang="ts">
