<!--
  One entity's audit log, as the sheet behind `EventMeta` shows it: the same cards the
...
<script lang="ts">
  ...
  interface Props {
     ...
     * per Zero emit reads as a change. `EventMeta` derives one off primitives for this.
     */
...
<QueryState offlineExcluded resource={feed.resource}>
  ...
  {#snippet empty()}
    <div class="space-y-1 py-10 text-center">
      ...
      {/if}
      <p class="text-surface-600-400 text-sm">{m.event_metaNoChanges()}</p>
    </div>
  {/snippet}
</QueryState>
