<!--
  ...
  hydration are `activityFeed()`, so the same component serves the global feed and any
  scoped one.
-->
<script lang="ts">
  ...
  interface Props {
    ...
    onLoadOlder?: () => void
    onMergeNew?: () => void
    /** The cards, newest first, already decided by `activityCard`. */
    views: readonly ActivityCardView[]
  }
