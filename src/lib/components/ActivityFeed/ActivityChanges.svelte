<!--
  ...
  What a line SAYS is decided by `activityChanges()` and arrives as a view; this file is the
...
<script lang="ts">
  ...
  import type { ActivityChangeView, PairFormat, SourceSide } from '$lib/entities/activity/change'
  ...
  interface Props {
    /** The card's changed columns, already decided by `activityChanges`. */
    changes: readonly ActivityChangeView[]
  }
...
{#snippet value(change: ActivityChangeView)}
  ...
  {:else if change.kind === 'prose'}
    ...
    <details class="min-w-0">
      <summary class="text-surface-600-400 cursor-pointer text-xs">
        {m.activity_compareCharacters({ count: (change.after ?? '').length })}
      </summary>
      ...
      <span class="flex flex-wrap items-center gap-1.5">
        ...
        {#each change.added as line (line.routeFk)}
          <span class="preset-tonal-success rounded-lg px-2 py-0.5 text-xs">
            {m.activity_changeTopoLineAdded({ route: lineName(line.name) })}
          </span>
        {/each}

        {#each change.redrawn as line (line.routeFk)}
          <span class="bg-surface-200-800 text-surface-950-50 rounded-lg px-2 py-0.5 text-xs">
            {m.activity_changeTopoLineRedrawn({ route: lineName(line.name) })}
          </span>
        {/each}

        {#each change.removed as line (line.routeFk)}
          <span class="preset-tonal-error rounded-lg px-2 py-0.5 text-xs">
            {m.activity_changeTopoLineRemoved({ route: lineName(line.name) })}
          </span>
        {/each}
      </span>
    </div>
  ...
  {:else if change.kind === 'file'}
    <span class="text-surface-600-400 text-xs">{m.activity_changeFileRemoved({ media: change.media })}</span>
  {:else}
