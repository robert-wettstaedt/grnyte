...
export type PointType = 'start' | 'middle' | 'top'
...
export class TopoEditor {
  ...
  #pendingSnapshot: { topoId: number; lines: EditLine[] } | undefined
  ...
  beginStroke(): void {
    if (this.topoId != null) this.#pendingSnapshot = { topoId: this.topoId, lines: cloneLines(this.currentLines) }
  }
  ...
  addLine(routeFk: number): void {
    this.#apply((lines) => {
      if (!lines.some((line) => line.routeFk === routeFk)) {
        lines.push({ routeFk, topType: 'top', points: [] })
      }
    })
    this.selectedRouteFk = routeFk
  }
  ...
  savedLinesFor(topoId: number): SavedLine[] {
    return this.lines(topoId)
      .filter((line) => line.points.length > 0)
      .map((line) => ({ routeFk: line.routeFk, topType: line.topType, path: serializePoints(line.points) }))
  }
}
