...
describe('canDeleteBlock', () => {
  it('lets a region DELETE holder remove anyone’s block', () => {
    expect(canDeleteBlock([region(1, REGION_PERMISSION_READ, REGION_PERMISSION_DELETE)], ME, { createdBy: SOMEBODY_ELSE, regionFk: 1 })).toBe(true)
  })

  it('lets an editor remove a block they created', () => {
    expect(canDeleteBlock([region(1, REGION_PERMISSION_READ, REGION_PERMISSION_EDIT)], ME, { createdBy: ME, regionFk: 1 })).toBe(true)
  })

  it('does not let an editor remove a block somebody else created', () => {
    expect(canDeleteBlock([region(1, REGION_PERMISSION_READ, REGION_PERMISSION_EDIT)], ME, { createdBy: SOMEBODY_ELSE, regionFk: 1 })).toBe(false)
  })

  it('does not carry the own-created grant across regions', () => {
    expect(canDeleteBlock([region(1, REGION_PERMISSION_READ, REGION_PERMISSION_EDIT)], ME, { createdBy: ME, regionFk: 2 })).toBe(false)
  })
