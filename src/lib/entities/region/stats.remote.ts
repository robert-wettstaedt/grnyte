 ...
 * the privileged branch then takes several more from the same ten-slot pool. Only the member
 * branch opens a transaction, and only for as long as its own reads take.
 */
export const regionStats = query(z.object({ regionFk: z.number() }), async ({ regionFk }): Promise<RegionStats> => {
  ...
  const stats = isMember
    ...
    : await collectRegionStats(db, regionFk, locale, now)
 ...
 *  A plain `query` on the privileged handle, like `listFeedback`: no transaction to hold open. */
export const listAllRegions = query(async (): Promise<RegionSummary[]> => {
  ...
  return collectRegionSummaries(db, getLocale())
})
