 ...
 * copy: these are the decisions that used to live in `ActivityChanges.svelte`, where the only
...
function change(partial: Partial<ActivityListItem>, ctx?: ActivityChangeContext): ActivityChangeView {
  const [only] = activityChanges([activity(partial)], ctx)
  return only
}
 ...
 */
function changeOf<K extends ActivityChangeView['kind']>(
  kind: K,
  partial: Partial<ActivityListItem>,
  ctx?: ActivityChangeContext,
): Extract<ActivityChangeView, { kind: K }> {
...
describe('activityChanges', () => {
  it('keeps only the columns the catalogue knows', () => {
    ...
    expect(activityChanges(rows).map((entry) => entry.id)).toEqual([1])
  })

  it('has none for a create row, which changed no column', () => {
    expect(activityChanges([activity({ type: 'created' })])).toEqual([])
  })
...
describe('location', () => {
  const pin = (partial: Partial<ActivityListItem>) =>
    change({ columnName: 'location', entityType: 'block', ...partial })

  it('says a pin was set when there was none before', () => {
    expect(pin({ newValue: stringifyCoords({ lat: 47.1, long: 8.5 }) })).toMatchObject({
      approximate: false,
      captionKey: 'activity_changeLocationSet',
      kind: 'location',
      metres: undefined,
    })
  })
  ...
  it('reports the distance in raw metres when the pin moved', () => {
    ...
    expect(moved.captionKey).toBe('activity_changeLocationMoved')
  ...
  it('reads confirmed when the pin stayed put', () => {
    ...
    ).toMatchObject({ captionKey: 'activity_changeLocationConfirmed', metres: undefined })
  })
  ...
  it('degrades to an update for a row written before coordinates were stored', () => {
    expect(pin({})).toMatchObject({ captionKey: 'activity_changeLocationUpdated', points: [] })
  })
  ...
  it('draws the old pin as gone when the location was cleared', () => {
    expect(
      ...
      captionKey: 'activity_changeLocationRemoved',
...
describe('topo', () => {
  const topo = (partial: Partial<ActivityListItem>, topos?: ReadonlyMap<number, TopoView>) =>
  ...
  it('names the four photo actions and lets a redraw speak through its chips', () => {
    expect(topo({ metadata: metadata('photoAdded', 700) }).captionKey).toBe('activity_changeTopoPhotoAdded')
    expect(topo({ metadata: metadata('photoReplaced', 700) }).captionKey).toBe('activity_changeTopoPhotoReplaced')
    expect(topo({ metadata: metadata('reordered') }).captionKey).toBe('activity_changeTopoReordered')
  ...
  it('falls back to a sentence when a redraw has no chips to show', () => {
    ...
    expect(topo({ metadata: metadata('lines', 700), newValue: lines, oldValue: lines }, topoView())).toMatchObject({
      captionKey: 'activity_changeTopoLinesUpdated',
    })
  })
