...
describe('activityMeta', () => {
  it('reads the newest row as an update', () => {
    ...
    ).toEqual({ actor: 'mara', key: 'activity_metaUpdated', timestamp: RECENT })
  })

  it('reads the entity s own creation row as a creation', () => {
    expect(
      activityMeta({ ...imported, latest: activity({ createdAt: RECENT, type: 'created', userName: 'mara' }) }),
    ).toEqual({ actor: 'mara', key: 'activity_metaCreated', timestamp: RECENT })
  })
  ...
  it('takes the preposition once the time reads as a date', () => {
    expect(activityMeta({ ...imported, latest: activity({ createdAt: OLD.getTime(), userName: 'mara' }) })?.key).toBe(
      'activity_metaUpdatedOn',
    )
  })

  it('takes the preposition on a dated creation with no actor', () => {
    expect(activityMeta({ ...imported, creatorName: undefined })?.key).toBe('activity_metaCreatedOnUnknown')
  })

  it('falls back to the entity s own stamp when nothing is logged', () => {
    expect(activityMeta(imported)).toEqual({
      actor: 'ada',
      key: 'activity_metaCreatedOn',
      timestamp: OLD.getTime(),
    })
  })

  it('drops the actor from the sentence while the row s user has not synced', () => {
    expect(activityMeta({ ...imported, latest: activity({ createdAt: RECENT, userName: '' }) })?.key).toBe(
      'activity_metaUpdatedUnknown',
    )
  })

  it('says nothing at all when there is neither a row nor a stamp', () => {
    expect(activityMeta({ ...imported, createdAt: undefined })).toBeUndefined()
  })
})
