...
import type { ActivityChangeView } from './change'
...
import { groupActivities, type ActivityGroup } from './grouping'
 ...
 * story module, so a column added to `ActivityListItem` had to be remembered four times, and a
...
export function activity(partial: Partial<ActivityListItem>): ActivityListItem {
  return {
...
export function changes(activities: ActivityListItem[], topos?: ReadonlyMap<number, TopoView>): ActivityChangeView[] {
  return activities.flatMap((entry) => view(groupActivities([entry])[0], undefined, undefined, topos).changes)
}

/** The map the feed hands cards: an absent key is still syncing, an explicit `null` is gone. */
export function entityMap(entries: [ActivityEntityRef, ActivityEntity | null][]): ActivityEntityMap {
