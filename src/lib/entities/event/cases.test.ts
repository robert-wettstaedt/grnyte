...
import { groups, view } from './catalogue.fixture'
import type { ChangeTopoLine } from './change'
import { eventEntityKey } from './entity'
 ...
 *
 * Then the snapshot: all 256 cases through the real `groupCatalogueRows` and `cardView`, which
...
function decided(entry: (typeof CASES)[number]) {
  return groups(entry.rows).map((group) => {
    const card: CardView = view(group, entry.entities ?? world, ME, entry.topos)
 ...
 */
function drawn(line: ChangeTopoLine) {
...
describe('activity case fixtures', () => {
  ...
  it('hydrates a tombstone for every whole-entity delete', () => {
    const deletes = CASES.filter((entry) =>
      entry.rows.some((activity) => activity.type === 'deleted' && activity.columnName == null),
    )

    expect(deletes.length).toBeGreaterThan(0)

    for (const entry of deletes) {
      for (const activity of entry.rows) {
        if (activity.type !== 'deleted' || activity.columnName != null) {
          continue
        }

        const key = eventEntityKey({ id: activity.entityId, type: activity.entityType })
