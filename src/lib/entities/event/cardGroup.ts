...
import { catalogueParentRef } from './entity'
 ...
 */
export type CardGroupKind = 'burst' | 'entity' | 'removal' | 'session' | 'single' | 'upload'
 ...
 */
export function groupCatalogueRows(rows: readonly CatalogueRow[]): CardGroup[] {
  const sorted = withoutRedundantSource([...rows].sort((a, b) => b.createdAt - a.createdAt || b.id - a.id))
  const groups: { group: CardGroup; key: string }[] = []
  // Only the newest group per key is still open; an older one is already out of window.
  const open = new Map<string, CardGroup>()
  ...
  for (const activity of sorted) {
    ...
    if (current != null && joins(current.rows[current.rows.length - 1], activity, kind)) {
  ...
  return mergeCreatedWithMedia(groups).map(({ group, key }) => ({
    ...group,
    id: `${key}#${oldestId(group.rows)}`,
    kind: group.rows.length === 1 ? ('single' as const) : group.kind,
  }))
}
 ...
 */
function groupKey(activity: CatalogueRow, kind: CardGroupKind): string {
...
function joins(oldest: CatalogueRow, activity: CatalogueRow, kind: CardGroupKind): boolean {
...
function kindOf(activity: CatalogueRow): CardGroupKind {
...
function localityKey(activity: CatalogueRow): string {
 ...
 */
function mergeCreatedWithMedia(groups: { group: CardGroup; key: string }[]): { group: CardGroup; key: string }[] {
  const merged = new Set<CardGroup>()
  ...
  const byCreate = new Map<string, { create: CatalogueRow; entry: (typeof groups)[number] }[]>()

  for (const entry of groups) {
    const creates = entry.group.rows.filter((activity) => activity.type === 'created')
    if (creates.length !== 1) {
      continue
    }
  ...
  for (const { group } of groups) {
    ...
    const parent = catalogueParentRef(group.rows[0])
    if (parent == null || !group.rows.every((activity) => sameParent(activity, parent))) {
      continue
    }
    ...
    if (target != null) {
      ...
      const rest = [...target.entry.group.rows.filter((activity) => activity !== target.create), ...group.rows]
      rest.sort((a, b) => b.createdAt - a.createdAt || b.id - a.id)
      target.entry.group.rows = [target.create, ...rest]
      merged.add(group)
    }
  }
 ...
 */
function oldestId(rows: readonly CatalogueRow[]): number {
  return rows.reduce((oldest, activity) => Math.min(oldest, activity.id), Infinity)
}

function sameParent(activity: CatalogueRow, parent: { id: string; type: string }): boolean {
...
function withinBurst(a: CardGroup, b: CardGroup): boolean {
 ...
 */
function withoutRedundantSource(sorted: CatalogueRow[]): CatalogueRow[] {
