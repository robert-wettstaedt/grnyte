 ...
 *
 * Six write sites, most of them about one column: a block is a name, a pin and a description, and
 * the pin can be set from three different screens (the edit form, the map picker, and a topo photo's
 * GPS). Same contract as `area.ts`: each case names its write site and claims what the card says.
 *
...
export const BLOCK_CASES: EventCase[] = [
...

  {
    ...
    id: 'BLOCK-02a',
    writer: 'blocks.remote.ts:208',
  },
  {
    action: '/blocks/400/edit -> Description -> Save',
    domain: 'block',
    events: [
      eventAgo(428, {
        actorFk: ME,
        changes: [
          change({
            ...
            newValue: 'Flat landing, out of the sun by three. The sit start is under the left arete.',
            oldValue: 'Flat landing, out of the sun by three.',
          }),
        ],
        objectId: 400,
        objectType: 'block',
    ...
    expected:
      'Single card, "You edited the description of Nordblock", the same shape AREA-02b has. The change line is prose, so it collapses behind a Compare toggle rather than showing a before/after pair inline. The form resubmits the name and the pin on every save and the diff drops both because neither moved, so a description edit writes one row.',
    id: 'BLOCK-02k',
    writer: 'blocks.remote.ts:208',
  },
  {
  ...
  },
  {
    ...
    events: [
      eventAgo(405, {
        ...
        // `{ description, location, name }`, and the rows are read back in the order they were written.
        changes: [
          change({ columnName: 'location', newValue: stringifyCoords(NUDGED), oldValue: stringifyCoords(PIN) }),
  ...
  },
  {
    ...
    expected:
      'Same headline and sub line as BLOCK-02f, "You edited Nordblock" with "2 edits", even though the reader used two screens. The only difference is the order of the two change lines, Name then Location here because the rows are read back in the order the two calls wrote them, against 02f\'s Location then Name from one submit diffing `{ description, location, name }`: nothing a reader could read as two screens, and the timestamp is the second call\'s. This is the server fold rather than the client grouping, so it holds even if the two land in different feed windows.',
    id: 'BLOCK-08b',
    writer: 'blocks.remote.ts:259',
  },
  {
