...
const ascent = (partial: Partial<CatalogueRow> = {}) =>
...
describe('groupCatalogueRows', () => {
  it('groups one climber s ascents from the same day into a session', () => {
    ...
    expect(groups[0].rows.map((a) => a.entityId)).toEqual(['10', '11'])
  ...
  it('groups one editor s crag edits under the same parent into a burst, and splits on the window', () => {
    ...
    expect(groups[0].rows.map((a) => a.entityId)).toEqual(['1', '2'])
    expect(groups[1].rows.map((a) => a.entityId)).toEqual(['4'])
  })
  ...
  it('keeps whole entity deletions out of the burst they would read as edits in', () => {
    ...
    expect(groups[0].rows.map((a) => a.entityId)).toEqual(['1', '2'])
    expect(groups[1].rows.map((a) => a.entityId)).toEqual(['1', '2'])
  })
  ...
  it('keeps a card s id when a newer activity joins it', () => {
    ...
    const before = groupCatalogueRows([older])
    const after = groupCatalogueRows([newer, older])
  ...
  it('keeps a source correction that landed inside the upload window', () => {
    ...
    expect(groups[0].rows.map((entry) => entry.columnName)).toEqual(['source'])
  })
  ...
  it('keeps a source edit made after the upload window as its own card', () => {
    ...
    expect(groups[0].rows.map((a) => a.columnName)).toEqual(['source'])
  })
  ...
  it('folds an upload into the create of the thing it landed on', () => {
    ...
    expect(groups[0].rows[0].type).toBe('created')
  })
  ...
  it('keeps a merged group chronological after the create it leads with', () => {
    ...
    expect(groups).toHaveLength(1)
    expect(groups[0].rows.map((entry) => entry.type)).toEqual(['created', 'updated', 'uploaded'])
  })
  ...
  it('keys a merged card on its oldest row, not on whatever sits last', () => {
    ...
    const [merged] = groupCatalogueRows([photo, create])
    const [alone] = groupCatalogueRows([create])
  ...
  it('returns nothing for no activities', () => {
    expect(groupCatalogueRows([])).toEqual([])
  })
})
