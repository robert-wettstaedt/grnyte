 ...
 * cases through the real `groupCatalogueRows` and `cardView`, so the cards can be judged by eye
...
import { activityAgo, entityMap, ME, photo, topoLines, topoMetadata, topos, video } from './catalogue.fixture'
import type { EventEntity, EventEntityMap, EventEntityRef } from './entity'
...
export type CaseDomain = 'area' | 'ascent' | 'block' | 'file' | 'region' | 'route' | 'topo' | 'user'
 ...
 */
const CRAG: [EventEntityRef, EventEntity | null][] = [
  [
    { id: '300', type: 'area' },
...
export const world: EventEntityMap = entityMap(CRAG)
...
export function fileEntity(name: string, files: ReturnType<typeof photo>[]): EventEntity {
  return { files, name, row: 'none' }
}

/** The crag with a few entries swapped, for the cases that need a tombstone or extra media. */
export function worldWith(entries: [EventEntityRef, EventEntity | null][]): EventEntityMap {
...
function pinnedBlock(estimated = false): EventEntity {
  return {
 ...
 */
function routeEntity(rawName: string, gradeFk: number): EventEntity {
...
export const CASES: Case[] = [
  ...
  },
  {
    ...
    id: 'ASC-04a',
    rows: [activityAgo(5, { columnName: 'rating', entityId: '9001', entityType: 'ascent', newValue: '2', userFk: ME })],
  },
  {
    ...
    id: 'ASC-04b',
    rows: [activityAgo(5, { columnName: 'rating', entityId: '9001', entityType: 'ascent', oldValue: '2', userFk: ME })],
  },
  {
  ...
  },
  {
    ...
    id: 'ASC-11c',
    rows: [activityAgo(5, { columnName: 'file', entityId: '9001', entityType: 'ascent', type: 'deleted', userFk: ME })],
  },
  {
  ...
  },
  {
    ...
    id: 'TOPO-01b',
    rows: [700, 701, 702, 703].map((topoId, index) =>
  ...
  },
  {
    ...
    id: 'TOPO-01d',
    rows: [700, 701, 702].map((topoId, index) =>
      activityAgo(5 + index * 3, {
        columnName: 'topo',
        entityId: '400',
        entityType: 'block',
  ...
  },
  {
    ...
    id: 'REG-05a',
    rows: [activityAgo(5, { columnName: 'role', entityId: '5', entityType: 'user', type: 'deleted', userFk: ME })],
  },
  {
  ...
  },
  {
    ...
    id: 'REG-07a',
    rows: [activityAgo(5, { columnName: 'membership', entityId: '5', entityType: 'user', type: 'deleted', userFk: 5 })],
  },
  {
