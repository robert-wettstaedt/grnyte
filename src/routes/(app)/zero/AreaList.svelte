<script lang="ts">
  ...
  const areas = $derived(
    new Query(page.data.z.current.query.areas.where('parentFk', 'IS', null).orderBy('name', 'asc')),
  )
...
{:else}
  <GenericList
    items={areas.current.map((item) => ({ ...item, id: item.id!, pathname: `/zero/${item.id}` }))}
  ...
  >
    {#snippet left(item)}
      ...
      {#if page.data.userRegions.length > 1}
    ...
    {#snippet right(item)}
      <div class="flex flex-col">
        <GradeHistogram
          axes={false}
          data={stats.current[item.id]?.grades ?? []}
          spec={{
            width: 100,
          }}
          opts={{
        ...
        <div class="flex items-center justify-end text-sm opacity-70">
          {stats.current[item.id]?.numOfRoutes ?? 0}

          {#if stats.current[item.id]?.numOfRoutes === 1}
            route
          {:else}
            routes
          {/if}
