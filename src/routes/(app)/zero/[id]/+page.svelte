<script lang="ts">
  ...
  const area = $derived.by(() => {
    const blocks = item?.current?.blocks.map((block) => {
      ...
      return { ...block, routes: sortedRoutes }
    })
  ...
  //           )
  //           .related('author')
  //           .related('ascents', (q) => q.orderBy('dateTime', 'desc').related('author').related('route'))
  //           .related('externalResources', (q) =>
  //             q.related('externalResource8a').related('externalResource27crags').related('externalResourceTheCrag'),
  //           )
  //           .related('grade')
  //           .related('files')
  ...
  let blocks = $state(area?.blocks)
  $effect(() => {
    blocks = area?.blocks
  })

  let files = $state(area?.files)
  $effect(() => {
    files = area?.files
  })
  ...
  const onChangeCustomSortOrder = async (items: typeof blocks) => {
    blocks = items

    const searchParams = new URLSearchParams()
    items?.forEach((item) => searchParams.append('id', String(item.id)))
    const res = await fetch(`/api/areas/${area?.id}/blocks/order?${searchParams.toString()}`, {
      method: 'PUT',
    })
                          ...
                          <div class="mt-2 flex flex-wrap gap-2">
                            {#each block.topos as topo}
                              <div class="flex flex-1 items-center justify-center md:flex-none md:justify-start">
                                <Image
                                  path={topo?.file?.path == null ? null : `/nextcloud${topo.file.path}/preview`}
                                  size={320}
                                />
                              </div>
                            {/each}
