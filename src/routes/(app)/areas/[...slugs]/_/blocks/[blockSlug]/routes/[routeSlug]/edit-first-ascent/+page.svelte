...
{:else}
  <ZeroQueryWrapper
    ...
    query={page.data.z.current.query.blocks
      ...
      .related('routes', (q) =>
        getRouteDbFilter(q).related('firstAscents', (q) => q.related('firstAscensionist', (q) => q.related('user'))),
      )
      .limit(1)}
  >
    {#snippet children([block])}
      {@const route = { ...block.routes[0], firstAscents: [...block.routes[0].firstAscents] }}
      ...
      {:else}
        <ZeroQueryWrapper
          loadingIndicator={{ type: 'skeleton' }}
          showEmpty
          query={page.data.z.current.query.firstAscensionists.orderBy('name', 'asc').related('user')}
        >
