<script lang="ts">
  ...
  const search = entitySearch({
    ...
    query: () => query,
    regionCrumb: (regionFk) => regionCrumb(global.userRegions, regionFk),
    regionFks: () => global.userRegions.map((region) => region.regionFk),
  })

  // Rows under the cursor for keyboard nav: recent queries when empty, else results.
  const count = $derived(showHistory ? history.length : search.flat.length)
...
<div bind:this={containerEl} class="relative w-full min-w-0">
  ...
  {#if open && (query.length > 0 || history.length > 0)}
